...
template <int DIM>
class Triangle
    : public ElementBase<DIM, Triangle<DIM>, TriangleNodes, TriangleFaces> {
 public:
  using BaseType =
      ElementBase<DIM, Triangle<DIM>, TriangleNodes, TriangleFaces>;
  Triangle() = default;
  Triangle(Index point_0, Index point_1, Index point_2);
 ...
 private:
  constexpr static short nFaces = 3;
  constexpr static short nNodes = 3;
  constexpr static short nNeighbors = 3;
  constexpr static short VTKtype = 5;
  constexpr static short maxNodesFace = 2;

  constexpr static short Faces[3][2] = {{0, 1}, {1, 2}, {2, 0}};

  constexpr static short NodesNeighbor[3][2] = {{1, 2}, {2, 0}, {0, 1}};

  constexpr static short nNodesFace[3] = {2, 2, 2};
  constexpr static short nNeighborNodes[3] = {2, 2, 2};
...
template <int DIM>
constexpr Index Triangle<DIM>::getMaxNodesFaceImpl() noexcept {
  return maxNodesFace;
}

template <int DIM>
constexpr Index Triangle<DIM>::getVTKtypeImpl() noexcept {
  return VTKtype;
}

template <int DIM>
Triangle<DIM>::Triangle(Index point_0, Index point_1, Index point_2) {
...
template <int DIM>
real Triangle<DIM>::getCoordCenterImpl(Index idim) const noexcept {
  assert(idim >= 0 && idim < DIM && "index out of range");
  return coordCenter[idim];
}

template <int DIM>
void Triangle<DIM>::setCoordCenterImpl(Index idim, real icoord) noexcept {
  assert(idim >= 0 && idim < DIM && "index out of range");
  coordCenter[idim] += icoord;
}

template <int DIM>
real Triangle<DIM>::getCoordFaceImpl(Index iface, Index idim) const noexcept {
...
template <int DIM>
void Triangle<DIM>::setCoordFaceImpl(Index iface, Index idim,
...
template <int DIM>
Index Triangle<DIM>::getNodesNeighborImpl(Index inode,
...
template <int DIM>
Index Triangle<DIM>::getnNodesFaceImpl(Index iface) noexcept {
  assert(iface >= 0 && iface < nFaces && "index out of range");
  return nNodesFace[iface];
}

template <int DIM>
Index Triangle<DIM>::getnNeighborNodesImpl(Index inode) noexcept {
  assert(inode >= 0 && inode < nNodes && "index out of range");
  return nNeighborNodes[inode];
}

template <int DIM>
Index Triangle<DIM>::getFacesImpl(Index iface, Index inode) noexcept {
  assert(iface >= 0 && iface < nFaces && "index out of range");
...
template <int DIM>
void Triangle<DIM>::changeOrientationImpl() noexcept {
  std::swap(this->nodes[0], this->nodes[2]);
}
}  // namespace preprocess