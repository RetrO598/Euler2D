...
//                   std::pow(param.PsOutlet / param.PtInlet, gam1 /
//                   param.gamma);
...
void yamlReader::read(parameter& param) {
  ...
  auto periodic = get("boundary_condition")["Periodic"];
  std::size_t nPeriodic = periodic.size();
  param.periodics.resize(nPeriodic);
  for (std::size_t i = 0; i < nPeriodic; ++i) {
    if (!periodic[i]["master"].IsNull() && !periodic[i]["slave"].IsNull()) {
      param.periodics[i].master_name = periodic[i]["master"].as<std::string>();
      param.periodics[i].slave_name = periodic[i]["slave"].as<std::string>();
      param.periodics[i].center =
          periodic[i]["rotation_center"].as<std::array<double, 2>>();
      param.periodics[i].rotate =
          periodic[i]["rotation"].as<std::array<double, 1>>()[0];
      param.periodics[i].translate =
          periodic[i]["translation"].as<std::array<double, 2>>();

      param.boundaryMap.insert(
          {param.periodics[i].master_name, BoundaryType::Periodic});
      param.boundaryMap.insert(
          {param.periodics[i].slave_name, BoundaryType::Periodic});

      param.periodicMaster.insert(param.periodics[i].master_name);
    }
  }
