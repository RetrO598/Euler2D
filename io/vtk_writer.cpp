...
// This function needs access to private members of FVMSolver (cv and timeSteps).
// A 'friend' declaration must be added to the FVMSolver class definition.
void writeVTKFile(const std::string& filename,
    ...
    if (consVars.size() != numPoints || timeSteps.size() != numPoints) {
        std::cerr << "VTK Writer Error: Mismatch between number of points (" << numPoints 
                  << ") and solution data size (" << consVars.size() << ")." << std::endl;
        return;
    }
    ...
    for (size_t i = 0; i < numPoints; ++i) {
        ...
        const double p = (solver.param.gamma - 1.0) * (cv.ener - 0.5 * (cv.xmom * u + cv.ymom * v));
